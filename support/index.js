            ...
            if (chatId === 173191632) {
                arr.forEach(({ user, hashrate, coin }) => {
                    ...
                    switch (user) {
                        case "mom":
                            userName = "Мама - ";
                            if (hashrate !== false && (coin === "eth" && hashrate < 55 || coin === "btg" && hashrate < 200)) {
                                resText += "******************************************\n";
                            }   
                            break
                        case "sanya":
                            if (coin === "eth" && hashrate < 330 || coin === "btg" && hashrate < 550) {
                                resText += "******************************************\n";
                                bot.sendMessage(
                                    478916730,
                                    "Скорость одной из монет(" + coin + ") упала - " + hashrate + "\r\n"
                                )
                            }
                            userName = "Саня - ";
                            break
                        case "me":
                            if (coin === "eth" && hashrate < 590 || coin === "zen" && hashrate < 500 || coin === "xvg" && hashrate < 15 || coin === "zcl" && hashrate < 250) {
                                resText += "******************************************\n";
                            }
                            userName = "Я - ";
                            break
